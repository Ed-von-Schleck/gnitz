...
GnitzDB utilizes a strict **Structure of Arrays (SoA)** architecture across all layers of the storage and execution engine. By unifying the in-memory format, the write-ahead log, the on-disk shards, and the DBSP execution operators around columnar batches, the system eliminates serialization overhead and format-conversion penalties.

### 1. In-Memory: ArenaZSetBatch (SoA)
The fundamental unit of data is the `ArenaZSetBatch`. Instead of packing rows into contiguous memory (AoS), records are decomposed into separate, contiguous arrays per column.
*   `pk_lo_buf`, `pk_hi_buf`, `weight_buf`, and `null_buf` represent the system columns.
*   `col_bufs[i]` holds the tightly packed primitive values for the $i$-th payload column.
*   `blob_arena` holds variable-length string tails in a unified German String layout.

### 2. Durability: WAL and Columnar Shards
Because the in-memory format is natively columnar, persistence is a zero-copy operation.
*   **WAL (Write-Ahead Log):** `wal_columnar.py` serializes `ArenaZSetBatch` column buffers directly to disk. Recovery consists of mapping or reading these column buffers straight back into memory.
*   **Table Shards:** `TableShardWriter` flushes MemTables to disk by laying out the exact same columnar buffers into an N-Partition shard file. 

### 3. Execution: DBSP Operators
DBSP operators process data **column-at-a-time (vectorized)** rather than row-at-a-time. 
*   By iterating over `col_bufs[i]` directly, operators maximize CPU cache locality and open the door for SIMD auto-vectorization by the C compiler beneath RPython.
*   I/O amplification is minimized: Queries that only touch a subset of columns will only page in the specific column buffers from the memory-mapped Shard files.

### 4. Row Abstraction (The Bridge)
While the physical layout and fast-paths are strictly columnar, the system provides `ColumnarBatchAccessor` and `RowBuilder` interfaces. These act as logical "row" lenses over the columnar data. 
*   **Warning:** Using `ColumnarBatchAccessor` to iterate over data row-by-row in hot paths (e.g., compaction or bulk filtering) destroys cache locality.
*   **Best Practice:** Always prefer column-wise loops over row-wise accessor loops when implementing new VM instructions or aggregations.

## Format Transition Map (Zero-Conversion)

```text
ArenaZSetBatch (Columnar buffers in memory)
    ├──→ WAL (Columnar blocks, fast sequential write)
    ├──→ MemTable (Accumulates columnar batches)
    │       └──→ Table Shard (Columnar mmap, zero-copy flush)
    │                                       ↑
...
| Target | Description |
|---|---|
| `make test` | Compile and run all RPython test suites (dev flags) |
| `make server` | Compile the server binary (`gnitz-server-c`, dev flags) |
| `make release-server` | Compile server with `--opt=jit --lto` (no debug metadata) |
| `make release-test` | Run full test suite with release flags |
...
Dev flags (`RPYFLAGS_DEV`): `--opt=1 --gc=incminimark --lldebug`.
Release flags (`RPYFLAGS_RELEASE`): `--opt=jit --gc=incminimark --lto`.
Override ad-hoc: `make server RPYFLAGS="--opt=2 --gc=incminimark"`.
