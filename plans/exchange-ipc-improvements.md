...
## Phase 2: WAL/IPC infrastructure

Each item is one commit; all independent.

### 2a. DDL WAL fan-out

**Files:** `gnitz/storage/wal.py`, `gnitz/server/master.py`, `gnitz/server/worker.py`,
`gnitz/server/ipc.py`

`broadcast_ddl` re-encodes the same batch for each IPC send. `WALWriter.append_batch`
gains `subscriber_fds: list`: after `encode_batch_to_buffer` writes to `block_buf`,
create one memfd, memmove from `block_buf`, then `send_fd` to each subscriber (encode
once, N+1 destinations: WAL file + N workers).

Delete `broadcast_ddl`, `_handle_ddl_sync`, `FLAG_DDL_SYNC`. Workers apply incoming
system-table blocks via `ingest_batch_memonly` + hook dispatch — already their existing
path for DDL replay.

No ACK barrier is needed. FIFO socket ordering guarantees workers process the DDL
block before any subsequent message sent on the same fd. The master sends DDL inline
with the next WAL flush and proceeds without waiting. The ACK barrier removal is the
primary value of this change: DDL-then-push no longer blocks on N round-trips. The
CPU savings from encoding once are negligible (DDL batches are 1–2 rows).

### 2b. Zero-copy WAL recovery

**Files:** `gnitz/storage/wal.py`

`WALReader.read_next_block()` does 2 `rposix.read` calls, 1 `lltype.malloc`, and two
byte-by-byte copy loops per block. Switch to mmap + `decode_batch_from_ptr` (already
used by `_recv_and_parse` in ipc.py — same call, same block format):
...
while off < file_size:
    ...
    yield batch
    batch.free()   # no-op: non-owning views into mmap
    off += block_size
mmap_posix.munmap_file(ptr, file_size)
```

Zero allocations, zero copies per block. This is ~15 lines reusing existing
primitives — the IPC and WAL paths already share the block format and decode function.

This change also validates the zero-copy decode path in WAL recovery tests before
Phase 5 relies on `decode_batch_from_ptr` in the production shared-memory hot path.

### 2c. Batch property flags on IPC

**Files:** `gnitz/server/ipc.py`

Add two flag bits in the high range (alongside `FLAG_HAS_SCHEMA` / `FLAG_HAS_DATA`):

```python
FLAG_BATCH_SORTED       = r_uint64(1 << 50)
FLAG_BATCH_CONSOLIDATED = r_uint64(1 << 51)
```

In `serialize_to_memfd`: set these bits from `zbatch._sorted` / `zbatch._consolidated`
when zbatch is not None.

In `_recv_and_parse`: after decoding the data block, restore:
```python
if payload.flags & FLAG_BATCH_SORTED:
    payload.batch._sorted = True
...
- **Push sub-batches** — after Phase 0, `repartition_batch` with `[pk_index]` from a
  consolidated source automatically marks sub-batches consolidated (flag propagation is
  inside `repartition_batch` and `multi_scatter`). No call-site changes needed.
- **FLAG_EXCHANGE messages from workers** — the pre-plan seal guarantees consolidated
  output. Workers set these flags on `do_exchange` sends. The master then knows it has
  N consolidated sorted runs (enabling Phase 3's `repartition_batches_merged`).
- **Relay sub-batches** — `repartition_batches_merged` marks its output consolidated
  internally. `relay_scatter` (Phase 0c) selects it automatically when all sources
  have `FLAG_BATCH_CONSOLIDATED`.

**Do NOT** propagate `_sorted = True` from sequential `repartition_batches` —
interleaved row order from N sources does not preserve PK monotonicity.

With Phases 0d + 2c + 3 + post-plan seal all in place, the zero-sort chain (see
...
Phase 2 ─── all unblocked, independent ─────────────────────────────────
  [2a] DDL WAL fan-out                 (wal.py, master.py, worker.py, ~35 lines)
  [2b] Zero-copy WAL recovery          (wal.py, ~15 lines)
  [2c] Batch property IPC flags        (ipc.py, ~20 lines; propagation already
                                        in exchange.py from Phase 0d)
       ↓ enables Phase 3
...
Each item is one commit. Run `make test 2>&1 | tee /tmp/test.log` after each.
...
**Phase 2:**
- 2a: `master_worker_test` — DDL sync path; verify workers see schema changes.
- 2b: `storage_comprehensive_test` — `test_zero_copy_wal_recovery` (new): write N
  blocks via WALWriter, recover via new WALReader, assert batch equality.
- 2c: `exchange_test` — add assertion that `_sorted` / `_consolidated` round-trip
  correctly through `serialize_to_memfd` / `_recv_and_parse`.
