...
### Unit 1: Automatic Compaction
**Plan:** `compaction.md` | **Impact:** Fixes unbounded shard growth (correctness + perf) | **~25 lines prod**

| File | Change |
|---|---|
| `core/store.py` | `compact_if_needed()` no-op on ZSetStore |
| `storage/ephemeral_table.py` | Import compactor, rename `create_cursor` → `_build_cursor`, new `create_cursor` with auto-compact, new `_compact()` |
| `storage/table.py` | Override `create_cursor()` (skip auto-compact), override `compact_if_needed()` via `execute_compaction` |
| `storage/partitioned_table.py` | Override `compact_if_needed()` — iterate partitions |
| `vm/runtime.py` | One line in `TraceRegister.refresh()` between cursor close and create |

**Tests:** `test_ephemeral_compaction` (storage), `test_compaction_through_ticks` (dbsp)
**Verify:** `make test | tail -200` — all existing tests pass unchanged
...
### Unit 3: Distinct Cursor Optimization
**Plan:** `distinct-optimizations.md` Opt 1 | **Impact:** O(|Δ|×K) → O((|Δ|+|H|)×log K) | **~15 lines**

| File | Change |
|---|---|
| `dbsp/ops/distinct.py` | Change signature to accept `hist_cursor` + `hist_table`; replace per-key `get_weight` with cursor seek |
| `vm/interpreter.py` | Pass `reg_history.cursor` and `reg_history.table` instead of just `reg_history.table` |

**Tests:** Existing distinct tests cover. No new tests required.
...
### Unit 7: Distinct Compile-Time Skip
**Plan:** `distinct-optimizations.md` Opt 2 | **~15 lines**

| File | Change |
|---|---|
| `vm/runtime.py` | `DeltaRegister.is_distinct = False` (1 line, compile-time only) |
| `catalog/program_cache.py` | Set `is_distinct = True` on REDUCE output; skip DISTINCT when input `is_distinct` |

**Tests:** New E2E test: REDUCE → DISTINCT circuit produces correct output with skip.
...
**Session 1:** Units 1+2+3 in parallel (all standalone, high-value)
**Session 2:** Unit 4 (foundational — unlocks Phases 3-5)
**Session 3:** Units 5+6+7 in parallel
**Session 4:** Unit 8 (join adaptive/merge-walk)
**Session 5:** Units 9+10 in parallel
**Session 6:** Unit 11, then Unit 12
**Sessions 7+:** Units 13-17 as needed
