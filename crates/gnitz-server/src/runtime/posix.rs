//! Linux IPC, socket and process-limit syscall wrappers used only by the
//! multi-process server: the eventfd/futex primitives the W2M rings park on,
//! the `MAP_SHARED` mappings those rings live in, the listener constructors,
//! and the unaligned accessors the SAL and W2M mmap paths read through.
//!
...
use gnitz_engine::foundation::posix_io::fd_size;
...
/// releasing the held W2M slot once the send completes. A peer that is already
/// gone (`ENOTCONN`) is the goal state, not an error, so every failure other
/// than `EINTR` is swallowed. Does NOT close the fd — the caller still reaps it
/// through the normal close path.
pub(crate) fn shutdown(fd: i32) {
    let _ = retry_eintr(|| unsafe { libc::shutdown(fd, libc::SHUT_RDWR) });
}
