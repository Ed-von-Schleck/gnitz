        ...
        let reservation = loop {
            ...
            match r {
                ...
                TryReserve::Full => {
                    ...
                    if !room_now {
                        let rc = posix::futex_wait_u32(hdr.writer_seq() as *const AtomicU32, expected, -1);
...
fn bump_and_wake(seq: &AtomicU32, flags: &AtomicU32, parked_bit: u32, site: &str) {
    ...
    let rc = posix::futex_wake_u32(seq as *const AtomicU32, 1);
    if rc < 0 {
        gnitz_fatal_abort!("{}: futex_wake_u32 failed: rc={} errno={}", site, rc, posix::errno());
    }
}
...
impl W2mReceiver {
    ...
    pub fn wait_any(&self, mut workers: u64, timeout_ms: i32) -> i32 {
        ...
        posix::futex_waitv_u32(&waiters[..n], timeout_ms)
    }
