    ...
    for (idx, sal_reader) in readers.iter().enumerate() {
        replayed += recover_sal(
            ...
            |cat, msg, data| {
                ...
                // must recover.
                if cat.dag().relation_kind(tid) == Some(gnitz_engine::query::RelationKind::Stream) {
                    return Ok(false);
                }
...
fn rebuild_invalid_views(catalog: &mut CatalogEngine, dispatcher: &MasterDispatcher) -> Result<(), String> {
    let invalid: Vec<i64> = catalog.invalid_views().collect();
...
mod zone_block_tests {
    ...
    use gnitz_engine_testkit::{make_batch, make_schema_u64_i64, SharedRegion};
