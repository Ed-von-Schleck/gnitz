...
/// Caller must free via `gnitz_ipc_wire_free`.
///
/// `batch_handle` may be null (no data). `schema_desc` may be null.
/// `col_names_ptr` / `col_names_len` / `num_col_names` pass column name strings.
#[no_mangle]
pub extern "C" fn gnitz_ipc_encode_wire(
    target_id: u64,
    client_id: u64,
    flags: u64,
    seek_pk_lo: u64,
    ...
    error_msg_len: u32,
    schema_desc: *const SchemaDescriptor,
    col_names_ptr: *const *const u8,
    col_names_len: *const u32,
    num_col_names: u32,
    ...
    let result = std::panic::catch_unwind(|| {
        let error_msg = if error_msg_ptr.is_null() || error_msg_len == 0 {
            &[] as &[u8]
        } else {
            unsafe { std::slice::from_raw_parts(error_msg_ptr, error_msg_len as usize) }
        };

        let schema = if schema_desc.is_null() {
            None
        } else {
            Some(unsafe { &*schema_desc })
        };

        let data_batch = if batch_handle.is_null() {
            None
        } else {
            Some(unsafe { &*(batch_handle as *const OwnedBatch) })
        };

        // Build column names slice
        let mut name_slices: Vec<&[u8]> = Vec::new();
        if !col_names_ptr.is_null() && !col_names_len.is_null() && num_col_names > 0 {
            let ptrs = unsafe { std::slice::from_raw_parts(col_names_ptr, num_col_names as usize) };
            let lens = unsafe { std::slice::from_raw_parts(col_names_len, num_col_names as usize) };
            for i in 0..num_col_names as usize {
                if ptrs[i].is_null() || lens[i] == 0 {
                    name_slices.push(b"");
                } else {
                    name_slices.push(unsafe {
                        std::slice::from_raw_parts(ptrs[i], lens[i] as usize)
                    });
                }
            }
        }
        let col_names = if name_slices.is_empty() { None } else { Some(name_slices.as_slice()) };

        let wire = encode_wire(
            target_id, client_id, flags,
            seek_pk_lo, seek_pk_hi, seek_col_idx,
            status, error_msg,
            schema, col_names, data_batch,
        );
...
/// Opaque decode result handle.
pub struct IpcDecodeResult {
    decoded: DecodedWire,
}

/// Decode a wire message. Returns an opaque handle.
/// Caller must free via `gnitz_ipc_decode_result_free`.
#[no_mangle]
pub extern "C" fn gnitz_ipc_decode_wire(
    data_ptr: *const u8,
    data_size: u32,
    out_status: *mut u32,
    out_client_id: *mut u64,
    out_target_id: *mut u64,
    out_flags: *mut u64,
    out_seek_pk_lo: *mut u64,
    out_seek_pk_hi: *mut u64,
    out_seek_col_idx: *mut u64,
    out_error_msg_ptr: *mut *const u8,
    out_error_msg_len: *mut u32,
    out_schema_desc: *mut SchemaDescriptor,
    out_has_schema: *mut i32,
    out_batch_handle: *mut *mut libc::c_void,
    out_num_col_names: *mut u32,
) -> *mut IpcDecodeResult {
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        unsafe {
            if !out_status.is_null() { *out_status = decoded.control.status; }
            if !out_client_id.is_null() { *out_client_id = decoded.control.client_id; }
            if !out_target_id.is_null() { *out_target_id = decoded.control.target_id; }
            if !out_flags.is_null() { *out_flags = decoded.control.flags; }
            if !out_seek_pk_lo.is_null() { *out_seek_pk_lo = decoded.control.seek_pk_lo; }
            if !out_seek_pk_hi.is_null() { *out_seek_pk_hi = decoded.control.seek_pk_hi; }
            if !out_seek_col_idx.is_null() { *out_seek_col_idx = decoded.control.seek_col_idx; }

            if !out_error_msg_ptr.is_null() && !out_error_msg_len.is_null() {
                if decoded.control.error_msg.is_empty() {
                    *out_error_msg_ptr = std::ptr::null();
                    *out_error_msg_len = 0;
                } else {
                    *out_error_msg_ptr = decoded.control.error_msg.as_ptr();
                    *out_error_msg_len = decoded.control.error_msg.len() as u32;
                }
            }

            if !out_has_schema.is_null() {
                *out_has_schema = if decoded.schema.is_some() { 1 } else { 0 };
            }
            if !out_schema_desc.is_null() {
                if let Some(ref sd) = decoded.schema {
                    *out_schema_desc = *sd;
                }
            }
            ...
            if !out_num_col_names.is_null() {
                *out_num_col_names = decoded.col_names.len() as u32;
            }
        }
...
/// Get a column name from a decode result.
#[no_mangle]
pub extern "C" fn gnitz_ipc_decode_result_col_name(
    handle: *const IpcDecodeResult,
    col_idx: u32,
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        if idx >= r.decoded.col_names.len() {
            unsafe {
        ...
        unsafe {
            *out_ptr = r.decoded.col_names[idx].as_ptr();
            *out_len = r.decoded.col_names[idx].len() as u32;
        }
        0
    });
    result.unwrap_or(-99)
...
/// Take ownership of the data batch from a decode result.
/// Returns null if no batch. The batch is removed from the result.
#[no_mangle]
pub extern "C" fn gnitz_ipc_decode_result_take_batch(
    handle: *mut IpcDecodeResult,
) -> *mut libc::c_void {
    if handle.is_null() { return std::ptr::null_mut(); }
    let result = std::panic::catch_unwind(|| {
        let r = unsafe { &mut *handle };
        match r.decoded.data_batch.take() {
            Some(batch) => Box::into_raw(batch) as *mut libc::c_void,
...
/// Free a decode result handle.
#[no_mangle]
pub extern "C" fn gnitz_ipc_decode_result_free(handle: *mut IpcDecodeResult) {
...
#[no_mangle]
pub extern "C" fn gnitz_atomic_load_u64(ptr: *const u8) -> u64 {
    if ptr.is_null() { return 0; }
    unsafe { atomic_load_u64(ptr) }
}

#[no_mangle]
pub extern "C" fn gnitz_atomic_store_u64(ptr: *mut u8, val: u64) {
    if ptr.is_null() { return; }
    unsafe { atomic_store_u64(ptr, val); }
}

#[no_mangle]
pub extern "C" fn gnitz_sal_write_group(
    sal_ptr: *mut u8,
    write_cursor: u64,
    num_workers: u32,
    target_id: u32,
    ...
    if sal_ptr.is_null() || worker_ptrs.is_null() || worker_sizes.is_null() {
        return SalWriteResult { status: -1, new_cursor: write_cursor };
    }
    unsafe {
        sal_write_group(
            sal_ptr, write_cursor, num_workers, target_id,
            lsn, flags, epoch, mmap_size, worker_ptrs, worker_sizes,
        )
    }
}

#[no_mangle]
pub extern "C" fn gnitz_sal_read_group_header(
    sal_ptr: *const u8,
    read_cursor: u64,
    worker_id: u32,
) -> SalReadResult {
...
#[no_mangle]
pub extern "C" fn gnitz_w2m_write(
    region_ptr: *mut u8,
    data_ptr: *const u8,
    data_size: u32,
    region_size: u64,
) -> i64 {
    if region_ptr.is_null() { return -1; }
    unsafe { w2m_write(region_ptr, data_ptr, data_size, region_size) }
}

#[no_mangle]
pub extern "C" fn gnitz_w2m_read(
    region_ptr: *const u8,
    read_cursor: u64,
    out_data_ptr: *mut *const u8,
    out_data_size: *mut u32,
) -> u64 {
    if region_ptr.is_null() || out_data_ptr.is_null() || out_data_size.is_null() {
        return read_cursor;
    }
    unsafe {
        let (new_rc, size, ptr) = w2m_read(region_ptr, read_cursor);
        *out_data_ptr = ptr;
        *out_data_size = size;
        new_rc
    }
