...
/// Build a valid DesiredState with one table, bincode it, and wrap it
/// in a MigrationRow suitable for `build_migration_row_batch` +
/// `ingest_to_family`. Helper shared across the fold tests.
fn make_migration_row_for_state(
    parent_hash: u128, state: &gnitz_wire::migration::DesiredState,
    author: &str, message: &str,
) -> super::migration::MigrationRow {
    use gnitz_wire::migration as mig;
    let canonical = mig::canonicalize(state);
    let hash = mig::compute_migration_hash(parent_hash, &canonical, author, message);
    super::migration::MigrationRow {
        hash, parent_hash,
        author: author.into(), message: message.into(),
        created_lsn: 1,
        desired_state_sql: String::new(),
        desired_state_canonical: canonical,
        format_version: 1,
...
#[test]
fn test_fold_migration_snapshot_empty_before_genesis() {
    let dir = temp_dir("fold_snapshot_empty");
    let mut engine = CatalogEngine::open(&dir).unwrap();
    assert_eq!(engine.current_migration_hash, 0);

    let snap = engine.fold_migration_snapshot().expect("fold must succeed");
...
#[test]
fn test_fold_migration_snapshot_reads_head_desired_state() {
    use gnitz_wire::migration as mig;
    let dir = temp_dir("fold_snapshot_head");
    let mut engine = CatalogEngine::open(&dir).unwrap();

    // Build a DesiredState with two tables and an index, ingest the
    // migration row directly (side-steps apply_migration; we only want
    // the sys_migrations row so the fold can read it).
    let state = mig::DesiredState {
        tables: vec![
            mig::TableDef {
                schema: "public".into(), name: "users".into(),
                columns: vec![mig::ColumnDef {
                    name: "id".into(), type_code: crate::schema::type_code::U64,
                    is_nullable: false,
                    fk_schema: String::new(), fk_table: String::new(), fk_col_idx: 0,
                }],
            ...
            mig::TableDef {
                schema: "public".into(), name: "orders".into(),
                columns: vec![mig::ColumnDef {
                    name: "id".into(), type_code: crate::schema::type_code::U64,
                    is_nullable: false,
                    fk_schema: String::new(), fk_table: String::new(), fk_col_idx: 0,
        ...
        views: vec![],
        indices: vec![mig::IndexDef {
            schema: "public".into(), name: "i_users_id".into(),
            owner_schema: "public".into(), owner_name: "users".into(),
            source_col: "id".into(), is_unique: false,
        }],
    };
    let row = make_migration_row_for_state(0, &state, "alice", "init");
    let batch = engine.build_migration_row_batch(&row);
    engine.ingest_to_family(MIGRATIONS_TAB_ID, &batch).unwrap();

    // Fold reads the head canonical and reconstructs the same shape.
    let snap = engine.fold_migration_snapshot().expect("fold must succeed");
    assert_eq!(snap.tables.len(), 2);
    assert!(snap.tables.contains(&("public".into(), "users".into())));
    assert!(snap.tables.contains(&("public".into(), "orders".into())));
    assert_eq!(snap.indices.len(), 1);
    assert!(snap.indices.contains(&(
        "public".into(), "i_users_id".into(),
...
#[test]
fn test_fold_migration_snapshot_deterministic() {
    use gnitz_wire::migration as mig;
    let dir = temp_dir("fold_snapshot_det");
    let mut engine = CatalogEngine::open(&dir).unwrap();

    let state = mig::DesiredState {
        tables: vec![mig::TableDef {
    ...
    let s1 = engine.fold_migration_snapshot().unwrap();
    let s2 = engine.fold_migration_snapshot().unwrap();
    let s3 = engine.fold_migration_snapshot().unwrap();
    assert_eq!(s1, s2, "fold must be deterministic");
    assert_eq!(s2, s3, "fold must be deterministic across repeated calls");
...
#[test]
fn test_fold_migration_snapshot_reads_latest_head_across_chain() {
    use gnitz_wire::migration as mig;
    let dir = temp_dir("fold_snapshot_chain");
    let mut engine = CatalogEngine::open(&dir).unwrap();

    // Chain: head starts at 0, write an initial migration, then a
    // second one that supersedes it. fold_migration_snapshot follows
    // current_migration_hash (updated by the MIGRATIONS_TAB_ID hook),
    // so after the second ingest it must reflect state2 only.
    let col_u64 = |n: &str| mig::ColumnDef {
        name: n.into(), type_code: crate::schema::type_code::U64,
        is_nullable: false,
        fk_schema: String::new(), fk_table: String::new(), fk_col_idx: 0,
    };
    let state1 = mig::DesiredState {
        tables: vec![mig::TableDef {
            schema: "public".into(), name: "a".into(),
            columns: vec![col_u64("id")],
            pk_col_idx: 0, unique_pk: true,
        }],
        views: vec![], indices: vec![],
    };
    let row1 = make_migration_row_for_state(0, &state1, "alice", "first");
    let b1 = engine.build_migration_row_batch(&row1);
    engine.ingest_to_family(MIGRATIONS_TAB_ID, &b1).unwrap();
    assert_eq!(engine.current_migration_hash, row1.hash);

    let state2 = mig::DesiredState {
        tables: vec![
            mig::TableDef {
                schema: "public".into(), name: "a".into(),
                columns: vec![col_u64("id")],
            ...
            mig::TableDef {
                schema: "public".into(), name: "b".into(),
                columns: vec![col_u64("id")],
                pk_col_idx: 0, unique_pk: true,
            },
        ],
    ...
    let row2 = make_migration_row_for_state(row1.hash, &state2, "alice", "second");
    let b2 = engine.build_migration_row_batch(&row2);
    engine.ingest_to_family(MIGRATIONS_TAB_ID, &b2).unwrap();
    assert_eq!(engine.current_migration_hash, row2.hash);

    let snap = engine.fold_migration_snapshot().unwrap();
    assert_eq!(snap.tables.len(), 2, "head must reflect state2, not union");
    assert!(snap.tables.contains(&("public".into(), "a".into())));
    assert!(snap.tables.contains(&("public".into(), "b".into())));
...
#[test]
fn test_dag_snapshot_missing_from_flags_unlisted_object() {
    let mut a = DagSnapshot::default();
    let mut b = DagSnapshot::default();

    a.tables.insert(("public".into(), "users".into()));
    a.tables.insert(("public".into(), "orders".into()));
    b.tables.insert(("public".into(), "users".into()));

    // `a` has one extra table → a.missing_from(b) must flag it.
    let diff = a.missing_from(&b);
    assert!(diff.is_some());
    let msg = diff.unwrap();
    assert!(msg.contains("orders"), "expected msg to name orders, got {}", msg);

    // Reverse: b ⊆ a, so b.missing_from(a) returns None.
    assert!(b.missing_from(&a).is_none());
}

#[test]
fn test_live_snapshot_excludes_system_and_fk_indices() {
    let dir = temp_dir("live_snap_excludes");
    let mut engine = CatalogEngine::open(&dir).unwrap();

    // Fresh DB: no user-visible objects yet. Live snapshot must be
    // empty even though sys_tables contains the catalog's own rows.
    let snap = engine.live_dag_snapshot();
    assert!(snap.tables.is_empty(),
        "live snapshot must skip system tables, got {:?}", snap.tables);
    assert!(snap.views.is_empty());
    assert!(snap.indices.is_empty());
