...
fn enforce_unique_pk_holds_at_every_pk_shape() {
    ...
    for case in cases {
        ...
        let eff = enforce_unique_pk(&mut pt, &schema, row(&k1, 2, 100));
        ...
        let eff = enforce_unique_pk(&mut pt, &schema, row(&k1, 2, 200));
        ...
        let eff = enforce_unique_pk(&mut pt, &schema, row(&k1, -3, 200));
        ...
        let eff = enforce_unique_pk(&mut pt, &schema, row(&k1, -1, 0));
        assert_eq!(eff.count, 0, "{name}: tombstoned-key retraction emits no phantom");
        let eff = enforce_unique_pk(&mut pt, &schema, row(&k2, -1, 0));
        ...
        let eff = enforce_unique_pk(&mut pt, &schema, churn);
        pt.ingest_owned_batch(eff).unwrap();
