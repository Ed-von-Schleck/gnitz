...
pub(crate) fn reads(instr: &Instr) -> [Option<u16>; 2] {
    match instr {
        Instr::Filter {
            in_reg,
            out_reg: _,
...
pub(crate) fn writes_state_during_replay(instr: &Instr) -> bool {
    match instr {
...
impl VmHandle {
    ...
    pub(crate) fn table(&self, idx: TableIdx) -> &Table {
        &self.tables[idx.at()]
    }
...
pub(crate) struct Program {
    ...
    pub reduce_plans: Vec<BakedReduce>,
...
impl Program {
    ...
    pub(crate) fn out_schema(&self) -> SchemaDescriptor {
...
pub(crate) struct RegisterFile {
    ...
    pub cursors: Vec<Option<Box<ReadCursor>>>,
}
