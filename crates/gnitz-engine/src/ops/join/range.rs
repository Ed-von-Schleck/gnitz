...
fn range_cut_points(eq: &[u8], d: &[u8], rel: RangeRel) -> Option<(PkBuf, Option<PkBuf>)> {
    let eq_size = eq.len();
    let slot_size = d.len();
...
///
/// One strategy, like the equi-join's symmetric co-group: the trace-driven
/// eq-group merge walk below. It seeks once per delta *group* rather than once
...
/// Differential oracle for [`range_merge_walk`]: the delta-driven per-row probe.
/// For each consolidated delta row, derive its `[start, end)` cut interval and
/// walk the matching trace run. Independent of the merge walk's grouping and
/// monotone-pointer machinery, so the two agreeing on the match multiset is real
/// evidence.
#[cfg(test)]
...
/// Trace-driven eq-group merge walk. Per delta eq-group, a single
/// `range_cut_points` cut on the group's extreme slot gives the one
/// `[start, end)` span covering the whole group; seek to `start` and sweep only
/// that span with a monotone delta pointer, emitting the matching contiguous
/// delta sub-range per trace row.
...
/// `g` = #delta groups, `covered` = matched trace rows (so `covered` is bounded
/// by the output, modulo weight-0 trace tombstones).
fn range_merge_walk(
    consolidated: &Batch,
    cursor: &mut ReadCursor,
    left_schema: &SchemaDescriptor,
