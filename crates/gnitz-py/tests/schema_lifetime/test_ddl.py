"""Binary-client DDL: creating and dropping a schema, a table, and the catalog
writes each rides one DDL_TXN frame.

The name-taken and not-found verdicts live in `admissibility/test_errors.py`;
the CREATE/DROP-against-a-lagging-worker race lives in
`interleaving/test_ddl_create_drop_race.py`.
"""
