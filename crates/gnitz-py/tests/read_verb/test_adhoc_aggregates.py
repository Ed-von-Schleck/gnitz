...
Run with GNITZ_WORKERS=4 (the fold is per-worker; the client merges partials):
    cd crates/gnitz-py && GNITZ_WORKERS=4 uv run pytest tests/read_verb/test_adhoc_aggregates.py -v --tb=short
"""
import random
...
def test_index_plan_keeps_the_exact_answer_and_a_repeatable_float_sum(tiny_ddl_chunk_server):
    ...
    The 3-row chunk is the point. `read_verb/test_index_reads.py::TestIndexBoundPushdown` covers
