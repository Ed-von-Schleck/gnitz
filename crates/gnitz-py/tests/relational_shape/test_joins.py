...
Run:
    cd crates/gnitz-py && GNITZ_WORKERS=4 uv run pytest tests/relational_shape/test_joins.py -v --tb=short
"""
import os
...
class TestJoins:
    ...
    def test_join_non_equi_range_accepted(self, client):
        ...
        (See tests/relational_shape/test_range_join.py for full distributed coverage.)"""
        sn = "s" + _uid()
        client.create_schema(sn)
        try:
            client.execute_sql(
