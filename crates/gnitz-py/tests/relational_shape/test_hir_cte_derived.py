...
def test_setop_cte_body(client):
    ...
    try:
        ...
        assert bag(scanned(client, sn, "v"), "a") == {(10,): 1, (20,): 1, (30,): 1}
        client.execute_sql("DELETE FROM u WHERE id = 2", schema_name=sn)  # drops a=30
        assert bag(scanned(client, sn, "v"), "a") == {(10,): 1, (20,): 1}
    finally:
...
def test_grouped_cte_body(client):
    ...
    try:
        ...
        assert bag(scanned(client, sn, "v"), "a", "n") == {(10, 2): 1, (20, 1): 1}
        client.execute_sql("INSERT INTO t VALUES (4, 20, 0)", schema_name=sn)
        assert bag(scanned(client, sn, "v"), "a", "n") == {(10, 2): 1, (20, 2): 1}
        client.execute_sql("DELETE FROM t WHERE id = 1", schema_name=sn)
        assert bag(scanned(client, sn, "v"), "a", "n") == {(10, 1): 1, (20, 2): 1}
    finally:
        client.drop_schema(sn)
...
def test_cte_in_subquery(client):
    ...
    try:
        ...
        assert bag(scanned(client, sn, "v"), "id") == {(1,): 1, (2,): 1, (3,): 1}
    finally:
        client.drop_schema(sn)
...
def test_setop_derived_body(client):
    ...
    try:
        ...
        assert bag(scanned(client, sn, "v"), "a") == {(10,): 1}
    finally:
        client.drop_schema(sn)
...
def test_grouped_over_derived(client):
    ...
    try:
        ...
        assert bag(scanned(client, sn, "v"), "a", "n") == {(10, 1): 1, (20, 1): 1}
    finally:
        client.drop_schema(sn)
...
def test_nested_derived(client):
    ...
    try:
        ...
        assert bag(scanned(client, sn, "v"), "a") == {(10,): 1, (30,): 1}
    finally:
        client.drop_schema(sn)
...
def test_linear_over_derived(client):
    ...
    try:
        ...
        assert bag(scanned(client, sn, "v"), "a") == {(20,): 1, (30,): 1}
        client.execute_sql("DELETE FROM t WHERE id = 2", schema_name=sn)
        assert bag(scanned(client, sn, "v"), "a") == {(30,): 1}
    finally:
        client.drop_schema(sn)
...
def test_count_star_over_join(client):
    ...
    try:
        ...
        client.execute_sql("DELETE FROM u WHERE id = 2", schema_name=sn)
        assert bag(scanned(client, sn, "v"), "n") == {(1,): 1}
    finally:
        client.drop_schema(sn)
