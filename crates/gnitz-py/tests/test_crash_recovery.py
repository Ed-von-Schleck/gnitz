...
def test_crash_after_push_no_tick(own_server):
...
def test_ddl_crash_no_orphan_columns(own_server):
    ...
    assert rc != 0, f"server should have aborted, got rc={rc}"
...
def test_ddl_crash_with_workers_no_orphans(own_server):
...
def test_ddl_crash_unique_index_foldin_rolls_back(own_server):
...
def test_orphan_drop_reclaimed_unflushed_create_survives(own_server):
...
def test_dropped_schema_subtree_reclaimed(own_server):
...
def test_recreated_schema_survives_recovery(own_server):
...
def test_worker_boot_flush_failure_preserves_sal(own_server):
    ...
    rc = own_server.start_expecting_exit(extra_env={"GNITZ_INJECT_BOOT_FLUSH_ERROR": "1"})
...
def test_master_sys_flush_failure_preserves_sal(own_server):
    ...
    rc = own_server.start_expecting_exit(extra_env={"GNITZ_INJECT_SYS_FLUSH_ERROR": "1"})
...
def test_unique_reinsert_after_restart(own_server):
...
def test_fk_no_orphan_after_restart(own_server):
...
def test_fk_restrict_update_after_restart(own_server):
...
def test_secondary_index_select_after_restart(own_server):
...
def test_ingest_apply_error_aborts_and_replays(inject, with_index, own_server):
