...
pub fn sql_type_to_typecode(dt: &DataType) -> Result<TypeCode, GnitzSqlParseError> {
    match dt {
        ...
        DataType::Boolean => Err(GnitzSqlParseError::Unsupported(
            "BOOLEAN has no gnitz type; use TINYINT(1)".to_string()
        )),
        _ => Err(GnitzSqlParseError::Unsupported(format!("unsupported SQL type: {:?}", dt))),
    }
}
