//! SQL → DesiredState parser for migrations.
//!
...
fn table_from_create(
    ...
    let pk_col_idx = pk_col_idx.ok_or_else(|| GnitzSqlParseError::Unsupported(format!(
...
fn index_from_create(
    ...
    let first_col = ci.columns.first().ok_or_else(|| GnitzSqlParseError::Unsupported(
