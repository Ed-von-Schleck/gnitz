...
fn a_misordered_bundle_backfills_cascades_on_drop_and_survives_a_rename() {
    ...
    let vids = client
        .create_view_chain(&sn, "f", misordered_chain(base_tid, &cols), None)
        .unwrap();
    ...
    let err = client.drop_table(&sn, "base").unwrap_err().to_string();
...
fn a_bundle_is_refused_whole_on_a_name_collision_or_over_the_segment_cap() {
    ...
    let taken = *client
        .create_view_chain(&sn, "taken", vec![segment(0, base_tid, &cols)], None)
        .unwrap()
        .last()
        .unwrap();

    let err = client
        .create_view_chain(&sn, "taken", misordered_chain(base_tid, &cols), None)
        .unwrap_err()
        .to_string();
    ...
    let err = client
        .create_view_chain(&sn, "x", planned, None)
        .unwrap_err()
        .to_string();
    assert!(
        err.contains(&format!(
