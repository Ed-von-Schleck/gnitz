...
use gnitz_core::{Connection, FIRST_USER_TABLE_ID, SCHEMA_TAB};
...
fn test_create_drop_schema() {
    ...
    let (_, data, _) = client.scan(SCHEMA_TAB).unwrap();
    let data = data.unwrap();
    let found = (0..data.len()).any(|i| data.weights[i] > 0 && data.pks.get(i) as u64 == sid);
    ...
    let still_present = (0..data2.len()).any(|i| data2.weights[i] > 0 && data2.pks.get(i) as u64 == sid);
...
fn test_string_columns() {
    ...
    let idx1 = (0..data.pks.len())
        .position(|i| data.pks.get(i) as u64 == 1)
    ...
    let idx2 = (0..data.pks.len())
        .position(|i| data.pks.get(i) as u64 == 2)
...
fn test_filter_view() {
    ...
    client
        .create_view_with_circuit("sv1", "filter_v", "", circuit, &table_schema.columns, &[0])
        .unwrap();
    ...
    let data = data.unwrap();
    let mut positive_pks: Vec<u64> = data
        .pks
        .to_vec_u128()
        .into_iter()
        .map(|x| x as u64)
    ...
    assert_eq!(
        positive_pks,
        vec![4, 5],
        "filter val>50 should keep pks 4 and 5; got {:?}",
        positive_pks
    );
...
fn test_reduce_view() {
    ...
    client
        .create_view_with_circuit("sv2", "reduce_v", "", circuit, &out_cols, &[0])
        .unwrap();
...
fn test_join_view() {
    ...
    let tid_a = client
        .create_table("sv3", "jt3a", &table_schema.columns, &[0u32], true, 0)
        .unwrap();
    let tid_b = client
        .create_table("sv3", "jt3b", &table_schema.columns, &[0u32], true, 0)
        .unwrap();
    ...
    for &pk in &[1u64, 2, 3] {
        ...
        if let ColData::Fixed(buf) = &mut batch_b.columns[1] {
    ...
    client
        .create_view_with_circuit("sv3", "join_v", "", circuit, &view_cols, &[0])
        .unwrap();
    ...
    for &pk in &[1u64, 4] {
        ...
        if let ColData::Fixed(buf) = &mut batch_a.columns[1] {
            buf.extend_from_slice(&0i64.to_le_bytes());
        }
    }
    ...
    let positive_pks: Vec<u64> = data
        ...
        .collect();
    assert!(
        positive_pks.contains(&1),
        "pk=1 should be in join output; got {:?}",
        positive_pks
    );
    assert!(
        !positive_pks.contains(&4),
        "pk=4 should not be in join output (no match in B)"
    );
...
fn test_anti_join_view() {
    ...
    let tid_a = client
        .create_table("sv4", "ajt4a", &table_schema.columns, &[0u32], true, 0)
        .unwrap();
    let tid_b = client
        .create_table("sv4", "ajt4b", &table_schema.columns, &[0u32], true, 0)
        .unwrap();
    ...
    client
        .create_view_with_circuit("sv4", "antijoin_v", "", circuit, &table_schema.columns, &[0])
        .unwrap();
    ...
    assert_eq!(
        positive_pks,
        vec![3],
        "anti-join should keep only pk=3 (no match in B); got {:?}",
        positive_pks
    );
...
fn test_incremental_update() {
    ...
    let tid = client
        .create_table("sv6", "iu6", &table_schema.columns, &[0u32], true, 0)
        .unwrap();
    ...
    client
        .create_view_with_circuit("sv6", "incr_v", "", circuit, &out_cols, &[0])
        .unwrap();
...
fn test_bulk_filter() {
    ...
    let circuit = cb.build();
    client
        .create_view_with_circuit("bf1", "bfv1", "", circuit, &table_schema.columns, &[0])
        .unwrap();
    ...
    assert_eq!(
        positive.len(),
        50_000,
        "filter val>50000 should keep 50000 rows; got {}",
        positive.len()
    );
...
fn test_bulk_exchange_multi_worker() {
    ...
    assert_eq!(
        positive_count, 500_000,
        "expected 500000 rows after exchange fan-out; got {}",
        positive_count
    );
...
fn test_left_join_view() {
    ...
    let tid_a = client
        .create_table("sv7", "lj7a", &schema_a.columns, &[0u32], true, 0)
        .unwrap();
    let tid_b = client
        .create_table("sv7", "lj7b", &schema_b.columns, &[0u32], true, 0)
        .unwrap();
    ...
    client
        .create_view_with_circuit("sv7", "lj_v", "", circuit, &out_cols, &[0])
        .unwrap();
    ...
    for &(pk, val_a) in &[(1u64, 10i64), (2, 20)] {
        ...
        batch_a.nulls.push(0);
        if let ColData::Fixed(buf) = &mut batch_a.columns[1] {
            buf.extend_from_slice(&val_a.to_le_bytes());
        }
    }
    ...
    let idx2 = (0..data.pks.len())
        ...
        .expect("row pk=2 missing");
    assert_ne!(
        data.nulls[idx2] & null_bit_val_b,
        0,
        "pk=2: val_b must be null (no match in B)"
    );
...
fn test_distinct_view() {
    ...
    let tid = client
        .create_table("sv8", "dt8", &table_schema.columns, &[0u32], true, 0)
        .unwrap();
    ...
    client
        .create_view_with_circuit("sv8", "distinct_v", "", circuit, &table_schema.columns, &[0])
        .unwrap();
...
fn test_min_max_aggregate_view() {
    ...
    let tid = client
        .create_table("sv9", "ma9", &table_schema.columns, &[0u32], true, 0)
        .unwrap();

    let agg_out_cols = |agg_name: &str| {
        vec![
            ColumnDef {
                name: "_hash".into(),
                type_code: TypeCode::U128,
    ...
    let vid_min = client.alloc_table_id().unwrap();
    {
        ...
        cb.sink(red);
        client
            .create_view_with_circuit("sv9", "min_v", "", cb.build(), &agg_out_cols("min_val"), &[0])
            .unwrap();
    }
    ...
    let vid_max = client.alloc_table_id().unwrap();
    {
        ...
        cb.sink(red);
        client
            .create_view_with_circuit("sv9", "max_v", "", cb.build(), &agg_out_cols("max_val"), &[0])
            .unwrap();
    }
