...
impl Peer {
    ...
    /// The target ids of the next `n` requests. Every one of them must be on its
    /// way **before the first reply goes out**, which is what makes this the
    /// batching assertion.
    fn expect_requests(&self, n: usize) -> Vec<u64> {
        (0..n).map(|i| self.expect_request(&format!("request {i}"))).collect()
    }
    ...
    /// A delta poll's answer: a control-only terminal frame whose watermark is
    /// `(tag, tick)`.
    fn reply_watermark(&self, tag: u64, tick: u64) {
        let h = Header {
...
/// One poll over M views writes M delta requests **before reading any reply**.
///
/// The peer answers nothing until it has read all M, so a sequential poll —
/// which would block on the first reply — never gets past its first request.
#[test]
fn one_poll_writes_every_request_before_it_reads_a_reply() {
    const M: usize = 6;
    ...
    let h = std::thread::spawn(move || {
        let mut ids = peer.expect_requests(M);
        peer.quiet("one request per view and no more");
        ids.sort_unstable();
        assert_eq!(ids, (100..100 + M as u64).collect::<Vec<_>>(), "every view is polled");
        for _ in 0..M {
            peer.reply_watermark(TAG, 9);
        }
        peer
    });
...
fn only_a_vanished_relation_pays_a_probe() {
    for (status, expected) in [(STATUS_NOT_FOUND, true), (STATUS_ERROR, false)] {
        ...
        let h = std::thread::spawn(move || {
            assert_eq!(peer.expect_request("the poll"), 7);
...
fn a_leftover_slot_does_not_shift_the_replies() {
    ...
    let h = std::thread::spawn(move || {
        let ids = peer.expect_requests(3);
        assert_eq!(ids[0], 7, "the abandoned scan is written first");
        // Its train, then one per view — each answered with a round derived from
        // the id that asked, so a shifted reply lands visibly wrong.
        peer.reply_watermark(0, 0);
        for &tid in &ids[1..] {
            peer.reply_watermark(TAG, 100 + tid);
        }
        peer
    });
    let report = client
...
fn no_recovery_runs_before_every_ingest_has() {
    ...
    let h = std::thread::spawn(move || {
        let ids = peer.expect_requests(2);
        peer.quiet("both polls go out together");
        // The first request's view is the one that vanished.
        peer.reply_status(ids[0], STATUS_NOT_FOUND, "gone");
        peer.reply_watermark(TAG, 11);
