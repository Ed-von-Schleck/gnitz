//! The meta-schema block across the two adapters that build it: `gnitz-core`'s
//! (client `Schema` → block) and `gnitz-engine`'s (`SchemaDescriptor` → block).
//!
//! It lives here because `gnitz-mirror` is the one crate that links both, and
//! because its own read path does exactly this conversion: `descriptor_of`
//! encodes a client `Schema` to a block and decodes it back as a
//! `SchemaDescriptor`, so a divergence between the two adapters is a live bug
...
proptest! {
    ...
    fn schema_block_bytes_agree_across_the_two_adapters(original in arb_schema(gnitz_wire::PK_LIST_MAX_COLS)) {
        ...
        let original = &original;
        let engine = encode_named_schema_block(original, &named_col_defs(&synthetic_names(original)), 7);
...
/// engine's own schema for that family — over seven real system schemas, with
