...
fn test_compound_pk_two_u64_accepted() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE cpk2 (a BIGINT UNSIGNED, b BIGINT UNSIGNED, payload BIGINT, PRIMARY KEY (a, b))")
            .unwrap();
    }
...
fn test_compound_pk_stride_12_accepted() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE pk_stride12 (a BIGINT UNSIGNED, b INT UNSIGNED, PRIMARY KEY (a, b))")
            .unwrap();
    }
...
fn test_compound_pk_string_column_rejected() {
    ...
    match err {
        GnitzSqlError::Unsupported(s) => {
            ...
            assert!(s.contains("'a'"), "expected offending column name in error, got: {}", s);
        }
        e => panic!("expected Unsupported, got {:?}", e),
    }
}
...
fn test_pk_float_column_rejected() {
    ...
    match err {
        GnitzSqlError::Unsupported(s) => {
            ...
            assert!(
                s.contains("'id'"),
                "expected offending column name in error, got: {}",
                s
            );
        }
        e => panic!("expected Unsupported, got {:?}", e),
...
fn test_table_level_pk_uppercase_matches_lowercase() {
    ...
    let (mut client, sn) = make_planner(&srv);
    {
        ...
        p.execute("CREATE TABLE case_pk (id BIGINT, name TEXT, PRIMARY KEY (ID))")
            .unwrap();
    }
...
fn test_group_by_nullable_u64_uses_synthetic_pk() {
    ...
    let (mut client, sn) = make_planner(&srv);
    {
        ...
        p.execute("CREATE TABLE evt (id BIGINT PRIMARY KEY, grp BIGINT UNSIGNED, val BIGINT NOT NULL)")
            .unwrap();
        p.execute("CREATE VIEW v_null_grp AS SELECT grp, COUNT(*) AS n FROM evt GROUP BY grp")
            .unwrap();
    }
...
fn test_group_by_nonnullable_u64_uses_natural_pk() {
    ...
    let (mut client, sn) = make_planner(&srv);
    {
        ...
        p.execute("CREATE TABLE evt2 (id BIGINT PRIMARY KEY, grp BIGINT UNSIGNED NOT NULL, val BIGINT NOT NULL)")
            .unwrap();
        p.execute("CREATE VIEW v_nat_grp AS SELECT grp, COUNT(*) AS n FROM evt2 GROUP BY grp")
            .unwrap();
    }
...
fn test_group_by_uuid_uses_natural_pk() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE evt3 (id BIGINT PRIMARY KEY, grp UUID NOT NULL, val BIGINT NOT NULL)")
            .unwrap();
        p.execute("CREATE VIEW v_uuid_grp AS SELECT grp, COUNT(*) AS n FROM evt3 GROUP BY grp")
            .unwrap();
    }
...
fn test_create_view_with_nullable_first_column_rejected() {
    use gnitz_core::{circuit::CircuitBuilder, ColumnDef};
    let srv = match ServerHandle::start() {
        Some(s) => s,
        None => return,
    };
    ...
    let msg = format!("{:?}", err);
    assert!(
        msg.to_lowercase().contains("nullable"),
        "expected nullable-PK error, got: {}",
        msg
    );
}
...
fn test_view_over_compound_pk_group_by_accepted() {
    ...
    create_compound_pk_table(&mut client, &sn, "cv_src3");
    {
        ...
        p.execute(
            "CREATE VIEW cv_gfull AS SELECT a, b, COUNT(*) AS n \
                   FROM cv_src3 GROUP BY a, b",
        )
        .unwrap();
        // Single-component grouping → single natural PK `[0]`.
        p.execute(
            "CREATE VIEW cv_gone AS SELECT a, COUNT(*) AS n \
                   FROM cv_src3 GROUP BY a",
        )
        .unwrap();
    }
...
fn test_view_over_compound_pk_too_wide_rejected() {
    ...
    assert!(
        client.resolve_table_or_view_id(&sn, "wv").is_err(),
...
fn test_view_over_compound_pk_join_source_accepted() {
    ...
    create_compound_pk_table(&mut client, &sn, "cv_jl");
    {
        ...
        p.execute("CREATE TABLE cv_jr (id BIGINT UNSIGNED PRIMARY KEY, v BIGINT)")
            .unwrap();
        p.execute("CREATE VIEW cv_j AS SELECT cv_jl.a, cv_jr.v FROM cv_jl JOIN cv_jr ON cv_jl.a = cv_jr.id")
            .unwrap();
    }
...
fn test_on_conflict_no_target_accepted_on_compound_pk() {
    ...
    let mut p = SqlPlanner::new(&mut client, &sn);
    p.execute("INSERT INTO conf_t2 (a, b, payload) VALUES (1, 2, 3) ON CONFLICT DO NOTHING")
        .unwrap();
}
...
fn test_column_level_unique_creates_index() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE u (id BIGINT PRIMARY KEY, val BIGINT UNIQUE)")
            .unwrap();
    }
    assert_eq!(
        unique_on(&mut client, &sn, "u", "val"),
        Some(true),
...
fn test_table_level_unique_single_column() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE ut (id BIGINT PRIMARY KEY, a BIGINT, UNIQUE(a))")
            .unwrap();
    }
...
fn test_named_unique_constraint_droppable_by_name() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE un (id BIGINT PRIMARY KEY, a BIGINT, CONSTRAINT u_a UNIQUE(a))")
            .unwrap();
    }
...
fn test_multi_column_unique_created_droppable_by_autoname() {
    ...
    p.execute("CREATE TABLE um (id BIGINT PRIMARY KEY, a BIGINT, b BIGINT, UNIQUE(a, b))")
        .unwrap();
...
fn test_lone_pk_unique_no_secondary_index() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE ul (id BIGINT PRIMARY KEY UNIQUE, v BIGINT)")
            .unwrap();
    }
...
fn test_compound_pk_member_unique_creates_index() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE uc (a BIGINT UNSIGNED, b BIGINT UNSIGNED, PRIMARY KEY(a, b), UNIQUE(a))")
            .unwrap();
    }
...
fn test_unique_plus_fk_same_column() {
    ...
    let (mut client, sn) = make_planner(&srv);
    {
        ...
        p.execute("CREATE TABLE chl (id BIGINT PRIMARY KEY, refc BIGINT UNIQUE REFERENCES par(x))")
            .unwrap();
    }
    assert_eq!(
        unique_on(&mut client, &sn, "chl", "refc"),
        Some(true),
...
fn test_self_ref_fk_before_inline_pk() {
    ...
    let (mut client, sn) = make_planner(&srv);
    {
        ...
        p.execute("CREATE TABLE sref (refc BIGINT REFERENCES sref(id), id BIGINT PRIMARY KEY)")
            .unwrap();
    }
    let s = schema_after_create(&mut client, &sn, "sref");
    assert!(
        s.columns[s.pk_index_single()].name.eq_ignore_ascii_case("id"),
...
fn test_drop_index_permitted_on_lone_pk_when_fk_referenced() {
    ...
    let (mut client, sn) = make_planner(&srv);
    {
        ...
        p.execute("CREATE TABLE cidx (cid BIGINT PRIMARY KEY, p BIGINT REFERENCES pidx(id))")
            .unwrap();
...
fn test_create_index_named_droppable_by_name() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE ni (id BIGINT PRIMARY KEY, col BIGINT)")
            .unwrap();
...
fn test_create_index_unnamed_droppable_by_autoname() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE ai (id BIGINT PRIMARY KEY, col BIGINT)")
            .unwrap();
...
fn test_create_index_reserved_infix_rejected() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE ri (id BIGINT PRIMARY KEY, col BIGINT)")
            .unwrap();
    }
    let mut p = SqlPlanner::new(&mut client, &sn);
...
fn test_create_index_invalid_identifier_rejected() {
    ...
    {
        let mut p = SqlPlanner::new(&mut client, &sn);
        p.execute("CREATE TABLE ii (id BIGINT PRIMARY KEY, col BIGINT)")
            .unwrap();
    }
    let mut p = SqlPlanner::new(&mut client, &sn);
