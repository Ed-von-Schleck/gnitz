...
fn schema(names: &[&str], pk_cols: &[usize]) -> Schema {
    Schema {
...
fn place_pk_front_pins_the_full_source_pk_to_the_leading_slots() {
    let rows: &[(&[&str], &[usize], &[Option<usize>], &[(&str, bool)], &[Option<usize>])] = &[
        (
            &["name", "age", "id"],
            &[2],
    ...
    for (names, pk_cols, srcs, want_cols, want_perm) in rows {
        ...
        for (slot, &pk) in s.pk_indices().iter().enumerate() {
            assert!(
                matches!(items[slot], ProjItem::PassThrough { src_col } if src_col == pk),
