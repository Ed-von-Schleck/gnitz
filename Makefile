...
rust-engine-test: ## Run only the gnitz-foundation + gnitz-store + gnitz-server tests (faster inner loop)
	cd crates && cargo test -p gnitz-foundation -p gnitz-store -p gnitz-server $(T)
