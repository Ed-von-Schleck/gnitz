...
from gnitz.storage.memtable_node import get_key_offset, node_get_payload_ptr, HASH_SIZE
from tests.row_helpers import create_test_row

# Constants for mocking MemTable node structure
# Simplified structure for testing: [8:Weight][1:Height][3:Pad][4*MAX_HEIGHT:Pointers][16:PK_Aligned][8:Hash][Payload]
...
class TestDataComparators(unittest.TestCase):
    def setUp(self):
        ...
        self.heap_buf = lltype.malloc(
            rffi.CCHARP.TO, 2048, flavor="raw"
        )  # Larger heap buffer for long string tests
    ...
    def _pack_aos_row_data(
        ...
        # Pack float or u128
        target_col_ptr = rffi.ptradd(
            ptr, payload_offset + schema.get_column_offset(2)
        )
        if is_u128_col:
            r_val = r_uint128(f_val_or_u128_val)
            rffi.cast(rffi.ULONGLONGP, target_col_ptr)[0] = r_uint64(r_val)
            rffi.cast(rffi.ULONGLONGP, rffi.ptradd(target_col_ptr, 8))[0] = r_uint64(
                r_val >> 64
            )
        else:
            rffi.cast(rffi.DOUBLEP, target_col_ptr)[0] = rffi.cast(
                rffi.DOUBLE, f_val_or_u128_val
            )
    ...
    def test_german_string_split_boundary(self):
        ...
        self.assertEqual(
            rffi.cast(lltype.Signed, off_ptr[0]), 999
        )  # Should be heap offset
    ...
    def test_string_equality_logic_unified(self):
        ...
        len1, pref1 = rffi.cast(lltype.Signed, u32_p1[0]), rffi.cast(
            lltype.Signed, u32_p1[1]
        )
        u32_p2 = rffi.cast(rffi.UINTP, self.struct_buf2)
        len2, pref2 = rffi.cast(lltype.Signed, u32_p2[0]), rffi.cast(
            lltype.Signed, u32_p2[1]
        )
        u32_p3 = rffi.cast(rffi.UINTP, struct_buf3)
        len3, pref3 = rffi.cast(lltype.Signed, u32_p3[0]), rffi.cast(
            lltype.Signed, u32_p3[1]
        )
        u32_p4 = rffi.cast(rffi.UINTP, struct_buf4)
        len4, pref4 = rffi.cast(lltype.Signed, u32_p4[0]), rffi.cast(
            lltype.Signed, u32_p4[1]
        )

        # "apple_sauce" > "apple_juice", result should be 1
        self.assertEqual(
            string_logic.compare_structures(
                len1,
                pref1,
                self.struct_buf1,
                self.heap_buf,
    ...
    def test_string_prefix_collision_and_ordering(self):
        ...
        len1, pref1 = (
            rffi.cast(rffi.UINTP, self.struct_buf1)[0],
            rffi.cast(rffi.UINTP, self.struct_buf1)[1],
        )
        ...
        len2, pref2 = (
            rffi.cast(rffi.UINTP, struct_buf_long)[0],
            rffi.cast(rffi.UINTP, struct_buf_long)[1],
        )
        ...
        len3, pref3 = (
            rffi.cast(rffi.UINTP, struct_buf_diff_char)[0],
            rffi.cast(rffi.UINTP, struct_buf_diff_char)[1],
        )
    ...
    def test_payload_ordering_and_equality_unified(self):
        ...
        p1_node, p1_offset = self._pack_aos_row_data(
            self.schema_i64, "apple", 1.0, self.heap_buf
        )
        p2_node, p2_offset = self._pack_aos_row_data(
            self.schema_i64, "banana", 1.0, self.heap_buf
        )
        p3_node, p3_offset = self._pack_aos_row_data(
            self.schema_i64, "apple", 1.0, self.heap_buf
        )
    ...
    def test_search_comparator_value_vs_packed(self):
        ...
        packed_node, _ = self._pack_aos_row_data(
            self.schema_i64, s_target, f_target, self.heap_buf
        )
        ...
        self.assertEqual(
            core_comparator.compare_rows(self.schema_i64, acc_packed, acc_val), 0
        )

        # Test for inequality (packed > value)
        acc_val.set_row(row_lt)
        self.assertEqual(
            core_comparator.compare_rows(self.schema_i64, acc_packed, acc_val), 1
        )

        # Test for inequality (value < packed)
        self.assertEqual(
            core_comparator.compare_rows(self.schema_i64, acc_val, acc_packed), -1
        )
    ...
    def test_u128_lexicographical_ordering(self):
        ...
        row1 = create_test_row(
            self.schema_u128,
            ["valA", (r_uint64(u128_max), r_uint64(u128_max >> 64))],
        )
        w.add_row_from_values(u128_1, 1, row1)

        row2 = create_test_row(
            self.schema_u128, ["valB", (r_uint64(u128_2), r_uint64(u128_2 >> 64))]
        )
        ...
        acc_soa.set_row(v, 0)
        row_comp = create_test_row(
            self.schema_u128,
            ["valA", (r_uint64(u128_3), r_uint64(u128_3 >> 64))],
        )
        ...
        self.assertEqual(
            core_comparator.compare_rows(self.schema_u128, acc_soa, acc_val), 1
        )
    ...
    def test_cross_shard_content_equality_long_strings(self):
        ...
        self.assertNotEqual(
            rffi.cast(lltype.Signed, v_a.blob_buf.ptr),
            rffi.cast(lltype.Signed, v_b.blob_buf.ptr),
        )
        ...
        self.assertEqual(
            core_comparator.compare_rows(self.schema_i64, acc_a, acc_b), 0
        )
