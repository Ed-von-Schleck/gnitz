...
class TestEphemeralTable(unittest.TestCase):
    ...
    def test_spill_to_disk_and_naming(self):
        """Verifies that data survives flushes (spills) and follows naming conventions."""
        key = r_uint128(99)
        ...
        # Implementation uses eph_shard_{tid}_{offset}.db
        self.assertIn(
            "eph_shard", shard_path, "Shard should follow ephemeral naming convention"
        )
    ...
    def test_scratch_table_creation(self):
        ...
        scratch = self.table.create_scratch_table(name, self.layout)

        try:
            self.assertIsInstance(scratch, EphemeralTable)
            self.assertEqual(scratch.name, name)
            # Implementation prepends "scratch_" to the directory name
            scratch_path = os.path.join(self.test_dir, "scratch_" + name)
            self.assertTrue(
                os.path.isdir(scratch_path), "Scratch directory should exist"
            )
        finally:
            scratch.close()
    ...
    def test_ghost_annihilation_on_flush(self):
        ...
        # Because the net weight was 0, the shard should exist but be empty
        if os.path.exists(shard_path):
            from gnitz.storage.shard_table import TableShardView

            view = TableShardView(shard_path, self.layout)
            self.assertEqual(
                view.count, 0, "Ghost record should not be materialized in shard"
            )
            view.close()
