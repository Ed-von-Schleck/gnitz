...
class AgeFilter(functions.ScalarFunction):
    """Filters rows where Age (Schema Col 2) >= 30."""
    def evaluate_predicate(self, row_accessor):
        # RowAccessor.get_int(idx) uses schema-based indexing
...
class AgeDoubler(functions.ScalarFunction):
    ...
    def evaluate_map(self, row_accessor, output_row):
        ...
        # resolve_string handles both raw storage structs and in-memory Python strings
        name = strings.resolve_string(res[2], res[3], res[4])
        output_row.append_string(name)
        
        # Col 2: Age -> Age * 2
        age = row_accessor.get_int(2)
        output_row.append_int(r_int64(age * 2))
...
class TestVMInterpreter(unittest.TestCase):
    ...
    def _mk_payload(self, name, age, schema=None):
        if schema is None:
            schema = self.schema
        return create_test_row(schema, [name, age])

    def _batch_to_dict(self, zbatch):
        """Converts batch to (Key, PayloadTuple) -> Weight for easy comparison."""
        res = {}
        schema = zbatch._schema
        for i in range(zbatch.length()):
            key = zbatch.get_pk(i)
            row = zbatch.get_row(i)
            
            # Extract non-PK columns as strings for comparison
            vals = []
            p_idx = 0
            for col_idx in range(len(schema.columns)):
                ...
                if col_type == types.TYPE_STRING.code:
                    vals.append(row.get_str(p_idx))
                else:
                    # Convert integers to string to match existing test assertions
                    vals.append(str(row.get_int(p_idx)))
                p_idx += 1
                
            payload_tuple = tuple(vals)
            weight = zbatch.get_weight(i)
            res[(key, payload_tuple)] = res.get((key, payload_tuple), 0) + int(weight)
        return res
    ...
    def test_basic_pipeline(self):
        ...
        in_batch.append(r_uint128(1), r_int64(1), self._mk_payload("Young", 20)) # Should be filtered
        in_batch.append(r_uint128(2), r_int64(1), self._mk_payload("Old", 40))   # Should pass and double
    ...
    def test_incremental_linearity(self):
        ...
        batch_a.append(r_uint128(10), r_int64(1), self._mk_payload("Alice", 35))
        
        # Delta B
        batch_b = batch.ZSetBatch(self.schema)
        batch_b.append(r_uint128(20), r_int64(1), self._mk_payload("Bob", 45))
        ...
        batch_total.append(r_uint128(10), r_int64(1), self._mk_payload("Alice", 35))
        batch_total.append(r_uint128(20), r_int64(1), self._mk_payload("Bob", 45))
    ...
    def test_integrate_feedback_loop(self):
        ...
        in_batch = batch.ZSetBatch(self.schema)
        in_batch.append(r_uint128(55), r_int64(1), self._mk_payload("Survivor", 55))
        in_batch.append(r_uint128(11), r_int64(1), self._mk_payload("Ghost", 11))

        view.process(in_batch)

        # Verify sink_db state
        # The Filter AgeFilter() passes Age >= 30.
        self.assertEqual(sink_db.get_weight(r_uint128(55), self._mk_payload("Survivor", 55)), 1)
        self.assertEqual(sink_db.get_weight(r_uint128(11), self._mk_payload("Ghost", 11)), 0)
        
        # Verify the context DB (db) remains empty (ensures sink isolation)
        self.assertEqual(self.db.get_weight(r_uint128(55), self._mk_payload("Survivor", 55)), 0)
    ...
    def test_incremental_join_logic(self):
        ...
        table_b.insert(r_uint128(100), self._mk_payload("Metadata_100", 0))
        table_b.flush()
        ...
        batch_a1.append(r_uint128(100), r_int64(1), self._mk_payload("Event_A", 99))
        
        res1 = view.process(batch_a1)
        results1 = self._batch_to_dict(res1)
        
        # Result should be join of Event_A and Metadata_100
        # Join Result Schema: [PK, NameA, AgeA, NameB, AgeB]
        ...
        table_b.insert(r_uint128(100), self._mk_payload("Metadata_New", 1))
    ...
    def test_retraction_logic(self):
        ...
        in_batch = batch.ZSetBatch(self.schema)
        in_batch.append(r_uint128(1), r_int64(1), self._mk_payload("Alice", 30))
        in_batch.append(r_uint128(2), r_int64(-1), self._mk_payload("Bob", 40))
