...
class BatchAccessor(row_logic.BaseRowAccessor):
    ...
    def set_row(self, payloads, index):
        """Batch specific: uses List[List[TaggedValue]] + index."""
    ...
    def get_u128(self, col_idx):
        val = self._get_val(col_idx)
        # Non-PK u128s are stored in i64 in TaggedValue
    ...
    def get_col_ptr(self, col_idx):
        """TaggedValues in a Batch are not memory-backed by raw pointers."""
        return NULL_PTR
...
class SortItem(object):
    ...
    def __lt__(self, other):
        ...
        res = row_logic.compare_records(
            self.batch.schema, self.batch.left_acc, self.batch.right_acc
        )
        return res < 0
...
class ZSetBatch(object):
    ...
    def get_payload(self, index):
        """Returns the payload list. Name restored for API compatibility."""
