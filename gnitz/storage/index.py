...
class ShardHandle(object):
    ...
    def __init__(self, filename, schema, min_lsn, max_lsn, validate_checksums=False):
        ...
        self.lsn = max_lsn
        self.view = shard_table.TableShardView(
            filename, schema, validate_checksums=validate_checksums
        )

        if self.view.xor8_filter is not None:
            self.xor8_filter = self.view.xor8_filter
        else:
        ...
        if self.view.count > 0:
            ...
            is_u128 = schema.get_pk_column().field_type.code == types.TYPE_U128.code
            k_min = (
                self.view.get_pk_u128(0)
                if is_u128
                else r_uint128(self.view.get_pk_u64(0))
            )
            k_max = (
                self.view.get_pk_u128(self.view.count - 1)
                if is_u128
