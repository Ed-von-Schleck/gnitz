...
class TableShardView(object):
    ...
    def __init__(self, filename, schema, validate_checksums=False):
        ...
        try:
            ...
            # Mmap the file for shared read-only access
            ptr = mmap_posix.mmap_file(fd, self.size, mmap_posix.PROT_READ, mmap_posix.MAP_SHARED)
            try:
                # Validation Barrier: check magic number before proceeding
    ...
    def _init_region(self, idx):
        ...
        # Eagerly validate Primary Key and Weight regions if checksums are enabled
