...
def compare_payloads(layout, ptr1, heap1, ptr2, heap2):
    for i in FIELD_INDICES:
        ...
        if ftype == types.TYPE_I64:
            ...
            p1 = rffi.ptradd(ptr1, foff); p2 = rffi.ptradd(ptr2, foff)
...
def skip_list_find_exact(base_ptr, head_off, entity_id, layout, encoded_payload_ptr, heap_ptr, update_offsets=None):
    ...
    for i in range(MAX_HEIGHT - 1, -1, -1):
        while True:
            ...
            if next_eid < entity_id: curr_off = next_off
            elif next_eid == entity_id:
                next_payload = node_get_payload_ptr(base_ptr, next_off)
                if compare_payloads(layout, next_payload, heap_ptr, encoded_payload_ptr, heap_ptr) < 0: curr_off = next_off
                else: break
            else: break
...
class MemTable(object):
    ...
    def __init__(self, layout, arena_size):
        self.layout = layout; self.arena = arena.Arena(arena_size); self.blob_arena = arena.Arena(arena_size)
        self.arena_size = arena_size; self.threshold_bytes = (arena_size * 90) / 100; self.rng = Random(1234)
        self._update_offsets = [0] * MAX_HEIGHT; self.arena.alloc(8)
        self.node_base_size = 12 + 8 + self.layout.stride; h_sz = self.node_base_size + (MAX_HEIGHT * 4)
        ptr = self.arena.alloc(h_sz); self.head_off = rffi.cast(lltype.Signed, ptr) - rffi.cast(lltype.Signed, self.arena.base_ptr)
        ptr[8] = chr(MAX_HEIGHT); node_set_weight(self.arena.base_ptr, self.head_off, 0)
    ...
    def has_active_data(self):
        base = self.arena.base_ptr; curr_off = node_get_next_off(base, self.head_off, 0)
        while curr_off != 0:
    ...
    def _pack_values_to_buf(self, dest_ptr, values):
        for i in FIELD_INDICES:
            ...
            val_obj = values[i]; f_type = self.layout.field_types[i]; f_off = self.layout.field_offsets[i]
            dest = rffi.ptradd(dest_ptr, f_off)
            if f_type == types.TYPE_STRING:
                if isinstance(val_obj, db_values.StringValue):
                    s_val = val_obj.v; l_val = len(s_val); heap_off = 0
                    if l_val > string_logic.SHORT_STRING_THRESHOLD:
                        blob_ptr = self.blob_arena.alloc(l_val); heap_off = rffi.cast(lltype.Signed, blob_ptr) - rffi.cast(lltype.Signed, self.blob_arena.base_ptr)
                        for j in range(l_val): blob_ptr[j] = s_val[j]
                    string_logic.pack_string(dest, s_val, heap_off)
            elif f_type == types.TYPE_I64:
                if isinstance(val_obj, db_values.IntValue): rffi.cast(rffi.LONGLONGP, dest)[0] = rffi.cast(rffi.LONGLONG, val_obj.v)
            elif f_type == types.TYPE_U64:
                if isinstance(val_obj, db_values.IntValue): rffi.cast(rffi.ULONGLONGP, dest)[0] = rffi.cast(rffi.ULONGLONG, val_obj.v)
    ...
    def upsert(self, entity_id, weight, field_values):
        ...
        try:
            ...
            self._pack_values_to_buf(tmp_buf, field_values); base = self.arena.base_ptr; blob_base = self.blob_arena.base_ptr
            pred_off = skip_list_find_exact(base, self.head_off, entity_id, self.layout, tmp_buf, blob_base, self._update_offsets)
            next_off = node_get_next_off(base, pred_off, 0); match_found = False
            if next_off != 0:
                if node_get_entity_id(base, next_off) == entity_id:
                    if compare_payloads(self.layout, node_get_payload_ptr(base, next_off), blob_base, tmp_buf, blob_base) == 0: match_found = True
            if match_found: node_set_weight(base, next_off, node_get_weight(base, next_off) + weight); return
            h = 1
            while h < MAX_HEIGHT and (self.rng.genrand32() & 1): h += 1
            node_sz = self.node_base_size + (h * 4); new_ptr = self.arena.alloc(node_sz); new_off = rffi.cast(lltype.Signed, new_ptr) - rffi.cast(lltype.Signed, base)
            node_set_weight(base, new_off, weight); new_ptr[8] = chr(h); eid_ptr = rffi.ptradd(new_ptr, 12 + (h * 4))
            rffi.cast(rffi.ULONGLONGP, eid_ptr)[0] = rffi.cast(rffi.ULONGLONG, entity_id); payload_dest = rffi.ptradd(eid_ptr, 8)
    ...
    def upsert_raw(self, entity_id, weight, raw_data):
        base = self.arena.base_ptr; blob_base = self.blob_arena.base_ptr
        tmp_buf = lltype.malloc(rffi.CCHARP.TO, self.layout.stride, flavor='raw')
        try:
    ...
    def flush(self, filename):
        sw = writer_ecs.ECSShardWriter(self.layout); base = self.arena.base_ptr; blob_base = self.blob_arena.base_ptr
        try:
            min_eid = r_uint64(0); max_eid = r_uint64(0); first = True
            curr_off = node_get_next_off(base, self.head_off, 0)
            while curr_off != 0:
                w = node_get_weight(base, curr_off)
                if w != 0:
                    eid = node_get_entity_id(base, curr_off); sw.add_packed_row(eid, w, node_get_payload_ptr(base, curr_off), blob_base)
                    if first or eid < min_eid: min_eid = eid
                    if first or eid > max_eid: max_eid = eid
                    first = False
                curr_off = node_get_next_off(base, curr_off, 0)
            sw.finalize(filename); return min_eid, max_eid
        finally:
            # Ensure buffers in the shard writer are freed even if an exception occurs
            sw.close()

    def free(self): self.arena.free(); self.blob_arena.free()

class MemTableManager(object):
    def __init__(self, layout, capacity, wal_writer=None, component_id=1):
        self.layout = layout; self.capacity = capacity; self.active_table = MemTable(self.layout, self.capacity)
        self.wal_writer = wal_writer; self.component_id = component_id; self.current_lsn = 1; self.starting_lsn = 1

    def put(self, entity_id, weight, field_values):
        lsn = self.current_lsn; self.current_lsn += 1
        if self.wal_writer is not None:
            data = self._pack_component_for_wal(field_values); self.wal_writer.append_block(lsn, self.component_id, [(entity_id, weight, data)])
        if self.active_table.arena.offset > self.active_table.threshold_bytes: raise errors.MemTableFullError()
    ...
    def flush_and_rotate(self, filename):
        min_eid, max_eid = self.active_table.flush(filename); self.active_table.free()
        self.active_table = MemTable(self.layout, self.capacity); self.starting_lsn = self.current_lsn; return min_eid, max_eid

    def _pack_component_for_wal(self, field_values):
        stride = self.layout.stride; buf = lltype.malloc(rffi.CCHARP.TO, stride, flavor='raw')
        try:
            for i in range(stride): buf[i] = '\x00'
            for i in FIELD_INDICES:
                if i >= len(field_values) or i >= len(self.layout.field_types): break
                val_obj = field_values[i]; f_type = self.layout.field_types[i]; dest = rffi.ptradd(buf, self.layout.field_offsets[i])
                if f_type == types.TYPE_STRING:
                    if isinstance(val_obj, db_values.StringValue): string_logic.pack_string(dest, val_obj.v, 0)
                elif f_type == types.TYPE_I64:
                    if isinstance(val_obj, db_values.IntValue): rffi.cast(rffi.LONGLONGP, dest)[0] = rffi.cast(rffi.LONGLONG, val_obj.v)
