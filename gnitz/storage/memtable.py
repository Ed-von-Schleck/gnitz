...
def calculate_node_size(height, key_len, val_len):
    # weight(8) + height(1) + pad(3) + next_pointers(h*4) + key + val
    size = 12 + (height * 4)
...
def node_get_key_str(base_ptr, node_off):
    ...
    while True:
        ...
        if not (b & 0x80): break
        shift += 7
    return rffi.charpsize2str(rffi.ptradd(p, idx), res)
...
class MemTable(object):
    ...
    def __init__(self, arena_size, threshold_percent=90):
        ...
        self.arena.alloc(8) # Offset 0 safeguard
    ...
    def flush(self, filename):
        ...
        while curr_off != 0:
            ...
            if w != 0:
                k = node_get_key_str(base, curr_off)
                v = node_get_val_str(base, curr_off)
                sw.add_entry(k, v, w)
...
class MemTableManager(object):
    ...
    def is_full(self, threshold=0.9):
        # The MemTable object now handles the logic efficiently
        return self.active_table.is_full()
