...
from gnitz.storage import mmap_posix, buffer, engine_ffi
...
class TableShardWriter(object):
    ...
    def finalize(self, filename, durable=True):
        ...
        _write_shard_via_ffi(
            mmap_posix.AT_FDCWD, filename, self.table_id, self.count,
            regions, durable=durable,
        )
        self.close()
...
def _write_shard_via_ffi(dirfd, name, table_id, count, region_list,
    ...
    dirfd: directory fd for openat/renameat, or AT_FDCWD for absolute paths.
    name: filename (relative to dirfd, or absolute if AT_FDCWD).
    """
