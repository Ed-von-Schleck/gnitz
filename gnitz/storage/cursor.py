...
# ---------------------------------------------------------------------------
# MemTableRowAccessor — reads from the last-found row of a Rust MemTable
# ---------------------------------------------------------------------------


class MemTableRowAccessor(core_comparator.RowAccessor):
    """Reads column data from the last-found row of a Rust MemTable handle.

    Follows the same pattern as ShardRowAccessor: per-column FFI calls.
    Only valid after MemTable.lookup_pk returned found=True and before
    the next mutation.
    """

    _immutable_fields_ = ["_schema", "_has_nullable"]

    def __init__(self, schema, memtable_handle):
        self._schema = schema
        self._has_nullable = schema.has_nullable
        self._handle = memtable_handle
    ...
    def refresh_null_word(self):
        ...
        self._null_word = r_uint64(
            engine_ffi._memtable_found_null_word(self._handle)
        )
    ...
    def _col_ptr(self, col_idx):
        ...
        stride = self._schema.columns[col_idx].field_type.size
        return engine_ffi._memtable_found_col_ptr(
            self._handle,
    ...
    def get_str_struct(self, col_idx):
        from gnitz.storage import engine_ffi
        ptr = self._col_ptr(col_idx)
        blob_ptr = engine_ffi._memtable_found_blob_ptr(self._handle)
...
class RustUnifiedCursor(AbstractCursor):
    ...
    def from_snapshots(schema, shard_views, snapshot_handles):
        ...
        snapshot_handles: list of opaque void* from MemTable.get_consolidated_snapshot().
        shard_views: list of _ShardPtrView (NOT owned by cursor).

        The cursor clones the snapshot Arc references internally.
        Caller must still free snapshot handles after closing the cursor.
        """
        from gnitz.storage import engine_ffi

        num_snaps = len(snapshot_handles)
        num_shards = len(shard_views)

        snap_arr = lltype.malloc(rffi.VOIDPP.TO, max(num_snaps, 1), flavor="raw")
        shard_arr = lltype.malloc(rffi.VOIDPP.TO, max(num_shards, 1), flavor="raw")

        for i in range(num_snaps):
            snap_arr[i] = snapshot_handles[i]
        for i in range(num_shards):
            shard_arr[i] = shard_views[i]._handle

        schema_buf = engine_ffi.pack_schema(schema)

        try:
            handle = engine_ffi._read_cursor_create_from_snapshots(
                snap_arr,
                rffi.cast(rffi.UINT, num_snaps),
                shard_arr,
                rffi.cast(rffi.UINT, num_shards),
                rffi.cast(rffi.VOIDP, schema_buf),
            )
        finally:
            lltype.free(snap_arr, flavor="raw")
            lltype.free(shard_arr, flavor="raw")
            lltype.free(schema_buf, flavor="raw")

        # Create the cursor object via __init__ with empty data,
        # then replace the handle.  RPython does not support __new__.
        cur = RustUnifiedCursor(schema, [], [])
        engine_ffi._read_cursor_close(cur._handle)
        cur._handle = handle
        cur._accessor = RustCursorAccessor(schema, handle, cur)
        cur._snapshot_handles = snapshot_handles

        # Estimated length from snapshot counts + shard counts
        est = 0
        for sh in snapshot_handles:
            est += intmask(engine_ffi._memtable_snapshot_count(sh))
        for sv in shard_views:
            est += sv.count
        cur._estimated_len = est

        # Initial seek to position 0
        out_valid = lltype.malloc(rffi.INTP.TO, 1, flavor="raw")
        out_key_lo = lltype.malloc(rffi.ULONGLONGP.TO, 1, flavor="raw")
        out_key_hi = lltype.malloc(rffi.ULONGLONGP.TO, 1, flavor="raw")
        out_weight = lltype.malloc(rffi.LONGLONGP.TO, 1, flavor="raw")
