...
from gnitz.storage.memtable_node import node_get_key, node_get_weight, node_get_next_off, node_get_payload_ptr
...
class BaseCursor(object):
    ...
    def seek(self, target_key): raise NotImplementedError
    ...
    def is_exhausted(self): raise NotImplementedError
    def peek_key(self): raise NotImplementedError
...
class MemTableCursor(BaseCursor):
    ...
    def __init__(self, memtable):
        ...
        self.exhausted = True
        self.accessor = comparator.PackedNodeAccessor(self.schema, memtable.blob_arena.base_ptr)
    ...
    def key(self):
        if self.exhausted: return r_uint128(-1)
        return node_get_key(self.memtable.arena.base_ptr, self.current_node_off, self.key_size)

    def weight(self):
        if self.exhausted: return r_int64(0)
        return node_get_weight(self.memtable.arena.base_ptr, self.current_node_off)

    def is_valid(self): return not self.exhausted
    def is_exhausted(self): return self.exhausted
    def peek_key(self): return self.key()
...
class ShardCursor(BaseCursor):
    ...
    def key(self):
        if self.exhausted: return r_uint128(-1)
        if self.is_u128: return self.view.get_pk_u128(self.position)
...
class UnifiedCursor(object):
    ...
    def __init__(self, schema, cursors):
        ...
        self.tree = tournament_tree.TournamentTree(self.cursors)
    ...
    def _find_next_non_ghost(self):
        while not self.tree.is_exhausted():
            ...
            while idx < num_candidates:
                curr = candidates[idx]
                if comparator.compare_rows(self.schema, curr.accessor, best_cursor.accessor) < 0:
                    best_cursor = curr
                idx += 1
            ...
            if net_weight != 0:
                ...
                self._current_accessor = best_cursor.accessor
                self._valid = True
                return
            else:
                idx = 0
                while idx < num_candidates:
                    curr = candidates[idx]
                    if comparator.compare_rows(self.schema, curr.accessor, best_cursor.accessor) == 0:
                        self.tree.advance_cursor_by_index(self._get_cursor_index(curr))
                    idx += 1
        self._valid = False
    ...
    def advance(self):
        ...
        candidates = self.tree.get_all_cursors_at_min()
        for c in candidates:
            if comparator.compare_rows(self.schema, c.accessor, target_accessor) == 0:
                self.tree.advance_cursor_by_index(self._get_cursor_index(c))
        self._find_next_non_ghost()

    def key(self): return self._current_key
    def weight(self): return self._current_weight
    def is_valid(self): return self._valid
    def get_accessor(self): return self._current_accessor
