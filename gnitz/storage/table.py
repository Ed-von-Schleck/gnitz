...
class PersistentTable(object):
        ...
        try:
            for block in reader.iterate_blocks():
                ...
                for rec in block.records:
                    self.memtable.upsert(rec.get_key(), rec.weight, rec.component_data)
    ...
    def _insert_weighted(self, pk, row, weight):
        ...
        rec = wal_format.WALRecord(r_uint128(pk), weight, row)
        self.wal_writer.append_block(lsn, self.table_id, [rec])
        
        # Visibility: Update MemTable
        self.memtable.upsert(r_uint128(pk), weight, row)
            ...
            for handle, row_idx in shard_matches:
                ...
                while idx < view.count:
                    if self.schema.get_pk_column().field_type.size == 16:
                        if view.get_pk_u128(idx) != r_key: break
                    else:
                        if r_uint128(view.get_pk_u64(idx)) != r_key: break
    ...
    def flush_memtable(self):
        ...
        acc = storage_comparator.PackedNodeAccessor(self.schema, self.memtable.blob_arena.base_ptr)
        ...
        while curr_off != 0:
            ...
            for i in range(len(self.schema.columns)):
                ...
                if col_type == types.TYPE_STRING:
                    ...
                    row.append_string(string_logic.unpack_string_from_struct(p, h))
                    ...
                    row.append_int(r_int64(rffi.cast(rffi.LONGLONG, acc.get_int(i))))
                payload_col += 1
    ...
    def close(self):
        ...
        if self.memtable: self.memtable.free()
        if self.index: self.index.close_all()
        self.is_closed = True
