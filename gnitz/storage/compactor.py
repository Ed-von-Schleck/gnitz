...
def compact_shards(input_files, output_file, schema, table_id=0, validate_checksums=False):
    ...
    # RPython: Avoid [None] * size for instances to keep annotations clean 
    # and prevent 'SomeNone' union issues. Use list append instead.
    views = []
    cursors = []
    ...
    try:
        ...
        while not tree.is_exhausted():
            ...
            for weight, exemplar_idx in merged:
                # Materialize the merged row into the new columnar shard
                # weight reflects the algebraic sum; exemplar_idx points to the source payload
...
def execute_compaction(table_id, policy, manifest_mgr, ref_counter, schema, output_dir=".", spine_obj=None, validate_checksums=False):
    ...
    Manages file lifecycle, manifest updates, and spine synchronization.
    """
    ...
    # RPython: String formatting requires intmask for large integers like LSNs
    lsn_val = intmask(shards[0].min_lsn)
    out_filename = os.path.join(output_dir, "compacted_%d_%d.db" % (table_id, lsn_val))
    
    # Lock input shards to prevent deletion by other reader threads during merge
    for f in input_files: 
        ref_counter.acquire(f)
        
    try:
        compact_shards(input_files, out_filename, schema, table_id, validate_checksums=validate_checksums)
        
        # Load the latest manifest to prepare an atomic update
        ...
        # Filter out the old superseded shards from the new manifest version
        new_entries = []
        ...
        # Open the new shard to verify bounds and calculate key ranges for the manifest
        ...
        try:
            ...
            if v.count > 0:
                ...
                # Edge case: All records in the range were annihilated
            ...
            # Atomic swap of the Manifest file
            manifest_mgr.publish_new_version(new_entries, shards[-1].max_lsn)
            
            # Update the in-memory Spine for immediate query performance improvement
            if spine_obj:
            ...
            # Mark superseded files for background deletion
            for f in input_files:
                policy.registry.unregister_shard(f)
                ref_counter.mark_for_deletion(f)
            
            # Register the new Guard Shard in the ShardRegistry
        ...
        # Release the read-locks; try_cleanup will physically delete unreferenced files
        for f in input_files: 
            ref_counter.release(f)
