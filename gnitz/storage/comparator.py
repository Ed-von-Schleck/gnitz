...
class RetractRowAccessor(RowAccessor):
    """Reads a single row from raw column pointers returned by retract_pk_row FFI."""

    _immutable_fields_ = ["schema"]

    def __init__(self, schema):
        self.schema = schema
        self._col_ptrs = lltype.nullptr(rffi.VOIDPP.TO)
        self._null_word = r_uint64(0)
        self._blob_ptr = NULL_PTR

    def set_ptrs(self, col_ptrs, null_word, blob_ptr):
        self._col_ptrs = col_ptrs
        self._null_word = null_word
        self._blob_ptr = blob_ptr
    ...
    def is_null(self, col_idx):
        ...
        return bool(r_uint64(self._null_word) & (r_uint64(1) << payload_idx))

    def _get_ptr(self, col_idx):
        payload_idx = col_idx if col_idx < self.schema.pk_index else col_idx - 1
        return rffi.cast(rffi.CCHARP, self._col_ptrs[payload_idx])
    ...
    def get_str_struct(self, col_idx):
        ptr = self._get_ptr(col_idx)
        if not ptr:
            return (0, 0, NULL_PTR, self._blob_ptr, None)
        ...
        if False:
            s = ""
        return (length, prefix, ptr, self._blob_ptr, s)
