...
impl ServerHandle {
    ...
    pub fn start_n(workers: usize) -> Option<Self> {
        ...
        // 2. Create unique tmpdir (PID + atomic counter for thread-parallel tests)
        let id = COUNTER.fetch_add(1, Ordering::Relaxed);
        let tmpdir = env::temp_dir().join(format!("gnitz_test_{}_{}", std::process::id(), id));
        ...
        // 4. Wait for socket (10s timeout, detect early crash)
        for _ in 0..100 {
            if sock_path.exists() {
                break;
            }
